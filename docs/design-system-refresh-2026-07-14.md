...
- **360px:** 라이브러리와 우봉고 실제 플레이 화면에서 가로 넘침 0, 44px 미만 버튼 0.
- **실제 2인 세션:** QA 봇을 두 번째 좌석에 연결해 인디언 포커, 타이거 앤 드래곤, 우봉고를 시작했다. 세 모듈 모두 올바른 `data-game-id`, 조작 가능한 상태, 깨진 이미지 0, 콘솔 warning/error 0을 확인했다.
...
- **모바일 플레이 순서:** 우봉고는 퍼즐→점수, 타이거 앤 드래곤은 현재 공격→내 손패/행동→공개 기록 순서로 바꿨다. 플레이 헤더의 일시정지·로비·나가기에는 520px 이하에서도 텍스트 라벨을 유지한다.
- **점수 레이아웃:** 타이거 앤 드래곤 점수판은 2/3/4인 수에 맞춰 열 개수를 조정하고, 우봉고의 점수 레일은 내용 높이까지만 차지한다.
...
- 전체 회귀에서 35개 인원별 시작 조합과 17개 게임 완주 시나리오를 모두 통과했다. 인디언 포커 2액션, 타이거 앤 드래곤 247액션, 우봉고 36액션을 포함해 각 게임이 종료·승자 확정까지 도달했다.
- 실제 1인 우봉고와 QA 봇을 연결한 2인 타이거 앤 드래곤·인디언 포커를 브라우저에서 시작하고 행동을 전송했다.
