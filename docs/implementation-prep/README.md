...
이 디렉터리는 조사 결과를 실제 구현 작업으로 옮기기 위한 기준 문서다. 코드 호환성을 위한 내부 ID와 독자 에셋을 사용하되, 사용자 노출명은 원작 게임명으로 표기한다.

| 조사 대상 | 내부 작업 ID | 사용자 노출명 | MVP 인원 | 구현 순서 |
|---|---|---|---:|---:|
| 인디언 포커 계열 | `blind-card-duel` | 인디언 포커 | 2 | 1 |
| 타이거 앤 드래곤 계열 | `parity-tile-duel` | 타이거 앤 드래곤 | 2–4 | 2 |
| 우봉고 계열 | `mosaic-rush` | 우봉고 | 1–4 | 3 |

내부 ID는 저장 데이터와 소켓 호환성을 위한 기술 식별자이며 화면에 노출하지 않는다.
